...
const ServiceSelection = ({ categories, services, selectedServices, toggleService }) => {
    ...
    const handleToggle = (id) => {
        toggleService(id);
        // Optional: Toast for selection? Maybe too noisy. Keeping it clean.
    };

    return (
        <div className="space-y-6 animate-fade-in-up">
            <div className="text-center">
                <h2 className="text-2xl font-serif font-bold text-[#3F0D12]">Select Services</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {services.filter(s => s.category === activeCategory).map(service => {
                    ...
                    ${isSelected
                                    ? 'border-[#D72638] bg-red-50 shadow-md transform scale-[1.02]'
                                    : 'border-transparent bg-gray-50 hover:bg-white hover:shadow-sm hover:border-gray-200'}`}
                        >
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors
                            ${isSelected ? 'bg-[#D72638] text-white' : 'bg-gray-200 text-gray-400'}`}>
                                    <Scissors size={18} />
                                </div>
                                <div className="overflow-hidden">
                                    <h4 className={`font-bold text-sm truncate ${isSelected ? 'text-[#D72638]' : 'text-gray-800'}`}>{service.name}</h4>
                                    <p className="text-xs text-gray-500">{service.duration_minutes} mins</p>
                                </div>
                            </div>

                            <div className="text-right">
                                <p className="text-sm font-black text-[#3F0D12]">₹{service.price}</p>
                                {isSelected && <CheckCircle size={16} className="text-[#D72638] ml-auto mt-1" />}
                            </div>
                        </div>
                    );
                })}
